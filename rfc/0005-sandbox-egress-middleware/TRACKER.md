...
- [done] Registration reload/removal semantics: gateway restart required (RFC 0003), running sandboxes re-sync on restart, removal of a referenced middleware causes those sandboxes to fail (`on_error`), runtime unavailability -> `on_error`.
...
- Registration reload semantics: gateway config is not hot-reloaded (RFC 0003 non-goal), so changing the registered middleware set requires a gateway restart. On restart, supervisors re-sync effective config, so running sandboxes pick up additions (no per-sandbox snapshot). Removing a middleware still referenced by an active policy will cause those sandboxes to fail (`on_error`, fail-closed); for now operators must remove references first, then the registration. Runtime unavailability of a registered middleware is handled the same way, by `on_error`. (Earlier draft claimed the gateway rejects such a config outright; softened, since cross-checking all active policies at config load is more mechanism than we want to commit to now.)

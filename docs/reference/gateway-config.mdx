...
# Effective global policy loaded from the policy source.
global_policy = "default"
# Set to "strict" to disable manual policy/provider mutation APIs.
enforcement = "strict"
...
`[openshell.gateway.policies]` configures an optional startup-loaded source for governed sandbox policies and provider type profiles. Set `location` to a filesystem directory, a `file://` URI, or a `grpc+unix://` Unix socket. Set `global_policy` to the policy document name the gateway should use as the effective global sandbox policy. When `global_policy` is configured, sandbox create requests cannot provide their own policy payloads and sandbox policy updates are rejected. Provider documents materialize source-backed provider records with the same names when their credentials can be resolved at runtime; attaching those providers composes governed provider policy rules into the effective policy.

Set `enforcement = "strict"` to also block runtime policy and provider mutation APIs. Strict mode allows attaching and detaching source-backed providers from sandboxes, but rejects manual policy updates, provider profile import/delete, provider instance create/update/delete, and provider credential refresh mutations.

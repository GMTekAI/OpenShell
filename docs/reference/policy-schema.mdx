...
| `lifecycle_operations` | list of strings | No | Closed, compiled fixed-path operation IDs that the gateway and supervisor may execute through `sandbox exec --lifecycle`. Arbitrary executable paths are not accepted. |
...
Lifecycle operations are static and fail closed unless the ID is compiled into the matching gateway and supervisor. The current registry includes `nemoclaw.hermes-mcp-config-transaction-v1`. That operation keeps the complete non-root workload identity and sandbox policy, authenticates the child through an inherited Unix socket using root `SO_PEERCRED` plus an exact one-shot handshake, accepts only a read-only probe or the fixed mutation payload shape, and runs over the private gateway-to-supervisor stream without a host TCP listener.

The supervisor requires the mapped helper and interpreter to be root-owned, non-writable, and FD-pinned so the running workload cannot replace them. This is runtime replacement protection, not image provenance or code signing. The operation receives no filesystem, process, network, or credential authority beyond the ordinary workload.
...
| `tls` | string | No | TLS handling mode. `auto` or an omitted value peeks at the first connection bytes and terminates detected TLS for inspected HTTPS traffic. `require` accepts only CONNECT tunnels that begin with a TLS ClientHello, requires certificate-validated TLS to the upstream, and fails closed without raw relay fallback. Plaintext HTTP, h2c, unsupported tunnel protocols, unavailable TLS state, and non-CONNECT forward requests are denied. Use `require` for credential-bearing authenticated HTTP and MCP endpoints. `skip` disables auto-detection for edge cases such as client-certificate mTLS or non-standard protocols. `terminate` and `passthrough` are deprecated aliases for `auto` and log a warning. Unknown values fail policy validation. When overlapping endpoint policies match, `require` takes precedence over weaker modes. |
...
| `credential_keys` | list of string | No | Up to 64 unique credential environment keys reserved for this authenticated MCP endpoint. Each key must match `^[A-Za-z_][A-Za-z0-9_]*$` and have only one owning endpoint in the sandbox policy. Requires `protocol: mcp`, `enforcement: enforce`, `tls: require`, and one literal `host`, `port`, and `path`. Wildcards, multiple ports, global policies, and provider-derived policy layers are rejected. Reservations are monotonic for the sandbox object's lifetime. A reserved key resolves only after a provider with an exact immutable ID and resource version is attached with the same key. Credential-bound MCP accepts exactly one `Authorization: Bearer <placeholder>` use site and rejects reserved markers elsewhere. |

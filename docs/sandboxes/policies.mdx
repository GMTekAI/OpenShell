...
process:
  ...
  # Optional closed maintenance IDs; arbitrary executables are never accepted.
...
| `process` | Static | Sets the OS-level identity for the agent process. `run_as_user` and `run_as_group` default to `sandbox`. Root (`root` or `0`) is rejected. The agent also runs with seccomp filters that block dangerous system calls. `lifecycle_operations` may grant only IDs from OpenShell's closed operation registry; it cannot name a shell, interpreter, or arbitrary executable. |
...
MCP endpoints must declare a concrete destination with `host` and `port` or `ports`. A policy entry that only sets `protocol: mcp` is invalid and is not treated as a wildcard MCP authorization. Use `path: /mcp` when the server's MCP endpoint is path-scoped; omitting `path` matches every HTTP path on that host and port. MCP request targets containing a query delimiter are rejected before credential replacement or any upstream write.

Set `tls: require` on credential-bearing authenticated MCP endpoints. This mode accepts only CONNECT tunnels that begin with a TLS ClientHello, requires certificate-validated TLS to the upstream, and denies plaintext HTTP, h2c, unsupported tunnel protocols, unavailable TLS state, and non-CONNECT forward requests. OpenShell does not fall back to a raw relay when required TLS setup or validation fails. Before credential replacement, OpenShell also requires an origin-form request target and exactly one valid `Host` header that matches the policy-bound host and HTTPS effective port.

Declare `credential_keys` only when the endpoint has one literal host, port, and path, uses `enforcement: enforce`, and sets `tls: require`. OpenShell durably reserves each declared key for the sandbox object's lifetime, including after policy removal. The key becomes active only when an exact provider ID and resource version is attached with the same credential key. It then resolves only as a single `Authorization: Bearer <placeholder>` value on a request authorized by that MCP endpoint. Global and provider-derived policies cannot declare `credential_keys`, and broader REST or L4 rules cannot authorize a reserved key.

...
Use `--output json` when automation must bind later mutations to the exact
object created by this request. The response includes the immutable provider
`id`, `resource_version`, type, and credential key names, but never credential
values.
...
Automation can make update fail closed if the name was replaced or the provider
changed after inspection:

```shell
openshell provider update my-claude \
  --expected-id <provider-id> \
...
For an ownership-sensitive delete, pass both `--expected-id` and
`--expected-resource-version`. OpenShell rejects the mutation if either value is
stale. Identity preconditions apply to one provider name per command.
...
This resolution requires the proxy to inspect HTTP. Configure the appropriate HTTP protocol, such as `protocol: rest` or `protocol: mcp`, and set `tls: require` for credential-bearing HTTPS endpoints. `tls: require` denies plaintext, h2c, unsupported tunnel protocols, unavailable TLS state, and non-CONNECT forwarding. It also requires certificate-validated upstream TLS without a raw relay fallback. Before replacement, OpenShell requires an origin-form request target and exactly one valid `Host` header matching the policy-bound host and HTTPS effective port. Endpoints with `tls: skip` pass encrypted traffic through as an opaque stream, so the proxy cannot resolve placeholders.
...
Credential-bound MCP is deliberately narrower than the general locations in
this table. When an enforced `protocol: mcp` endpoint declares
`credential_keys`, OpenShell resolves exactly one authorized placeholder in an
`Authorization: Bearer` header. It rejects duplicate authorization fields and
reserved credential markers in the target, query, session header, or custom
headers. The provider must be attached with its exact immutable ID, current
resource version, and the same credential key before resolution is active.

...
| What you can change | Set `tls: require` to accept only CONNECT tunnels that begin with a TLS ClientHello and require certificate-validated TLS to the upstream. Set `tls: skip` to disable TLS detection and termination for client-certificate mTLS or non-standard binary protocols. |
| Risk if relaxed | Auto-detect permits plaintext HTTP when policy otherwise allows it. `tls: skip` also disables placeholder credential rewriting, dynamic token grant injection, and L7 inspection for that endpoint. The proxy relays encrypted traffic without seeing the contents. |
| Recommendation | Use `tls: require` for credential-bearing authenticated HTTP and MCP endpoints. It denies plaintext, h2c, unsupported tunnel protocols, unavailable TLS state, and non-CONNECT forwarding without raw relay fallback. Before credential replacement, OpenShell requires origin-form and exactly one valid `Host` header matching the policy-bound host and HTTPS effective port. Use `tls: skip` only when the upstream requires the client's own TLS certificate or uses a non-HTTP protocol. |
...
| Maintenance operations | `lifecycle_operations` accepts only compiled operation IDs. Gateway and supervisor both revalidate the operation and payload; execution uses the complete workload identity, the same Landlock/seccomp boundary, a one-shot supervisor-authenticated descriptor, no provider environment, no shell/TTY/stdin, and a hard timeout over the private stream. |
...
| Allowing TLS downgrade on credential-bearing APIs | Auto-detect permits plaintext HTTP, while `tls: skip` disables placeholder credential rewriting, dynamic token grant injection, and L7 inspection. | Use `tls: require` for credential-bearing authenticated HTTP and MCP. Use `tls: skip` only when the upstream requires client-certificate mTLS. |

...
Both methods install the latest stable release by default. To install a specific version, set `OPENSHELL_VERSION` for the binary installer or pin the version with `uv tool install openshell==<version>`.
...
See [`deploy/helm/openshell/README.md`](deploy/helm/openshell/README.md) for available versions and configuration.
...
**Manual gateway binary:**

Use this when you want to run the gateway directly instead of installing a package-managed service. Choose a release tag from [OpenShell GitHub Releases](https://github.com/NVIDIA/OpenShell/releases), then download the gateway artifact for your host:
...
Generate local mTLS materials for the gateway:
...
Configure the gateway environment before starting it:

| Variable | Value | Purpose |
| -------- | ----- | ------- |
| `OPENSHELL_BIND_ADDRESS` | `127.0.0.1` | Listen on loopback for a local gateway. |
| `OPENSHELL_SERVER_PORT` | `17670` | Gateway API and SSH proxy port. |
| `OPENSHELL_TLS_CERT` | `${OPENSHELL_TLS_DIR}/server/tls.crt` | Server certificate generated by `generate-certs`. |
| `OPENSHELL_TLS_KEY` | `${OPENSHELL_TLS_DIR}/server/tls.key` | Server private key generated by `generate-certs`. |
| `OPENSHELL_TLS_CLIENT_CA` | `${OPENSHELL_TLS_DIR}/ca.crt` | CA used to verify mTLS clients. |
| `OPENSHELL_DB_URL` | `sqlite:${OPENSHELL_STATE_DIR}/gateway/openshell.db` | Gateway state database. |
| `OPENSHELL_GRPC_ENDPOINT` | `https://127.0.0.1:${OPENSHELL_SERVER_PORT}` | Callback endpoint exposed to sandbox supervisors. |
| `OPENSHELL_SSH_GATEWAY_HOST` | `127.0.0.1` | Hostname advertised for SSH proxy connections. |
| `OPENSHELL_SSH_GATEWAY_PORT` | `${OPENSHELL_SERVER_PORT}` | Port advertised for SSH proxy connections. |
| `OPENSHELL_SSH_HANDSHAKE_SECRET` | output of `openssl rand -hex 32` | Shared secret for gateway-to-sandbox SSH handshakes. Persist this value across restarts. |
| `OPENSHELL_DRIVERS` | `docker` or `podman` | Compute driver used to launch sandbox containers. |
| `OPENSHELL_SANDBOX_IMAGE` | `ghcr.io/nvidia/openshell-community/sandboxes/base:latest` | Default sandbox image. |
| `OPENSHELL_DOCKER_TLS_CA` | `${OPENSHELL_TLS_DIR}/ca.crt` | Docker driver CA mounted into sandboxes. |
| `OPENSHELL_DOCKER_TLS_CERT` | `${OPENSHELL_TLS_DIR}/client/tls.crt` | Docker driver client certificate mounted into sandboxes. |
| `OPENSHELL_DOCKER_TLS_KEY` | `${OPENSHELL_TLS_DIR}/client/tls.key` | Docker driver client key mounted into sandboxes. |
| `OPENSHELL_SUPERVISOR_IMAGE` | `ghcr.io/nvidia/openshell/supervisor:${VERSION#v}` | Podman driver supervisor image. |
| `OPENSHELL_PODMAN_TLS_CA` | `${OPENSHELL_TLS_DIR}/ca.crt` | Podman driver CA mounted into sandboxes. |
| `OPENSHELL_PODMAN_TLS_CERT` | `${OPENSHELL_TLS_DIR}/client/tls.crt` | Podman driver client certificate mounted into sandboxes. |
| `OPENSHELL_PODMAN_TLS_KEY` | `${OPENSHELL_TLS_DIR}/client/tls.key` | Podman driver client key mounted into sandboxes. |

For Docker, set the Docker TLS variables. For Podman, enable the user socket with `systemctl --user enable --now podman.socket`, set `OPENSHELL_DRIVERS=podman`, and set the Podman TLS variables.
